...
@php
    $classes = 'block mt-1 w-full peer h-[3rem] px-6 text-sm text-text bg-primary-background rounded-lg border-border border-2 outline-none placeholder-gray-300 placeholder-opacity-0 transition duration-200 placeholder-transparent placeholder:pointer-events-none
...
<x-form.field class="flex relative">
    ...
    <div class="overflow-hidden absolute right-2 -top-1 translate-y-1/2">
        <a type="button" @click.prevent="navigator.clipboard.writeText({{ $value }}), showMsg = true, setTimeout(() => showMsg = false, 1000)" class="group inline-flex items-center justify-center text-lg font-normal text-center text-slate-500 truncate rounded-b transition-colors hover:text-slate-400">
